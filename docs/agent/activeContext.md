...
- 2026-09-03：全量审计发现并修复一个排序边界 bug（git f6cae8b）：测试中卡片按旧排序键停位，但新落地卡片插入时误按测试中卡片的实时状态比较，可能把 400ms 卡片固化在 150ms 卡片前。插入比较现在统一使用有效排序键；浏览器复验通过。审计还确认快照清理、空列表状态机、Electron 错误路径和网页版均无问题。
