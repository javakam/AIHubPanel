...
## 构建 / 运行命令
- 运行：`node server.mjs`（默认端口 4179）
- 日常启动：双击 start-aihubpanel.bat（端口 4398，就绪后自动开浏览器）
- 桌面 exe 部分尚未开工，Go 的编译、打包命令在里程碑 0 之后补充到这里

## 关键依赖
- 无第三方依赖：前端原生 JS，服务端只用 Node 内置模块
- Node.js 18+
- 桌面方案会用 WebView2 Runtime（Win10/11 自带，无需安装）
